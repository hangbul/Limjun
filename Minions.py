...
# Minion Type
GOBLIN_KNGIHT, GOBLIN_SPEAR, GOBLIN_BABARIAN, DWARF_WORRIR = range(4)
...
class IdleState:
    ...
    @staticmethod
    def draw(minion):
        minion.image.clip_draw(int(minion.frame) * minion.w, 0, minion.w, minion.h, 0 - minion.window_left + minion.x,  0 - minion.window_bottom + minion.y)
...
class AttackState:
    ...
    @staticmethod
    def do(minion):
        minion.frame = (minion.frame + FRAMES_PER_ACTION * ACTION_PER_TIME * game_framework.frame_time) % 28
        minion.x = clamp(25, minion.x, 1600 - 25)
        if minion.state == 0:
